class R extends Font {
  Donut donut;
  R(float s, PVector p,color c) {
    super(s, p,c);
    donut = new Donut(s/4f, new PVector(size/25 + s/4f, size - size/10f), s/6f, 180, -90, (int)2);
    donut.setColor(c);
  ...
  public R(float s,PVector p){this(s,p,color(1,1,1,1));}
